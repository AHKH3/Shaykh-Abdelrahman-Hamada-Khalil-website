document.addEventListener('DOMContentLoaded', function() {
    ...
    function getNextLang() {
        return document.documentElement.lang === 'ar' ? 'en' : 'ar';
    }

    if (languageSwitcher) {
        languageSwitcher.addEventListener('click', function(e) {
            // إذا كان الضغط على زر اللغة أو النص فقط
            if (e.target === languageSwitcher || e.target === currentLangDisplay) {
    ...
    // عند تغيير حجم الشاشة، حدث نص الزر
    window.addEventListener('resize', function() {
        updateLangButtonText(document.documentElement.lang);
    });

    // عند تحميل الصفحة، حدث نص الزر حسب اللغة
    updateLangButtonText(document.documentElement.lang);
    ...
    document.querySelectorAll('.dropdown-content a').forEach(item => {
        item.addEventListener('click', function(e) {
            ...
            setTimeout(() => langDropdown.style.display = '', 200);
        });
    });
