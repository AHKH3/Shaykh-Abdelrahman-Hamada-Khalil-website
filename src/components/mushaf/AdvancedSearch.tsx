...
export default function AdvancedSearch({
  ...
  return (
    ...
    >
      ...
      <div data-testid="mushaf-index-panel" className="flex min-h-0 flex-1 flex-col">
        {/* Header & Search Input */}
              ...
              {showFilters && (
                ...
                >
                  <div className="space-y-1.5">
                    <label className="mushaf-text-overline font-bold uppercase tracking-wider text-muted-foreground px-1">{t.mushaf.searchInSurah}</label>
                    <select
                      value={selectedSurah || ""}
                  ...
                  <div className="space-y-1.5">
                    <label className="mushaf-text-overline font-bold uppercase tracking-wider text-muted-foreground px-1">{t.mushaf.searchInJuz}</label>
                    <select
                      value={selectedJuz || ""}
              ...
              {results.length > 0 ? (
                <>
                  <div className="px-5 py-2.5 sticky top-0 bg-background/80 backdrop-blur-md z-10 flex items-center justify-between border-b border-border/20">
                    <span className="mushaf-text-overline font-bold text-muted-foreground uppercase tracking-wider">
                      ...
                      {result.type === "surah" ? (
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1 min-w-0 whitespace-nowrap">
                            ...
                            <p className="mushaf-text-meta text-muted-foreground whitespace-nowrap">{t.mushaf.page} {result.pageNumber}</p>
                          </div>
                        </div>
                      ) : (
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            <span className="mushaf-text-overline px-2 py-0.5 bg-muted rounded font-mono text-muted-foreground" dir="ltr">
                              {result.verseKey}
                            </span>
                            <span className="mushaf-text-overline text-muted-foreground">
                ...
                {activeTab === "surah" ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {chapters.map((ch) => (
                      ...
                      >
                        <div className="w-9 h-9 flex-shrink-0 flex items-center justify-center rounded-lg bg-muted mushaf-text-compact font-bold text-muted-foreground group-hover:bg-primary/10 group-hover:text-primary transition-colors">
                          {ch.id}
                        </div>
                        <div className="flex-1 min-w-0 whitespace-nowrap">
                          ...
                          <div className="mushaf-text-overline flex items-center gap-2 text-muted-foreground whitespace-nowrap">
                        ...
                        <div className="text-end opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                          <span className="mushaf-text-compact font-bold text-primary">
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
                    {juzList.map((juz) => (
                      ...
                      >
                        <span className="mushaf-text-overline text-muted-foreground group-hover:text-primary transition-colors uppercase font-bold tracking-widest">{locale === "ar" ? "جزء" : "Juz"}</span>
